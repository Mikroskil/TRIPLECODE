<?php
	...
	$in=mysql_query("insert into user values('','$nama','$password','$alamat','$email','$telp','0','$tgl','$jk','$lahir','$provinsi','$kota','$pos','$fax')");
		if($in){
		echo "<script>alert('Registrasi Sukses ! Silahkan Login...');window.location.href='../register.php';</script>";
		}
		else{
		echo "<script>alert('Something wrong, pls try again !');javascript:history.go(-1);</script>";
		}
	}
